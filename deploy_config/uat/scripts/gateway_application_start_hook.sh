...
docker run -d --name INSERT_SERVICE_NAME --add-host host.docker.internal:host-gateway \
    ...
    -e KONG_PG_PORT:INSERT_POSTGRE_KONG_GATEWAY_PORT -e KONG_PG_SSL:on -e KONG_PROXY_LISTEN="0.0.0.0:INSERT_MAIN_HTTP_PORT, 0.0.0.0:INSERT_MAIN_HTTPS_PORT ssl" -e KONG_DATABASE=postgres \
    -e KONG_ADMIN_LISTEN="0.0.0.0:INSERT_API_HTTP_PORT, 0.0.0.0:INSERT_API_HTTPS_PORT ssl" -e KONG_ADMIN_GUI_LISTEN="0.0.0.0:INSERT_KONG_GATEWAY_HTTP_PORT, 0.0.0.0:INSERT_KONG_GATEWAY_HTTPS_PORT ssl" \
    -e KONG_AMMIN_GUI_URL="INSERT_KONG_DASHBOARD_ENDPOINT" -e KONG_PG_HOST=INSERT_POSTGRE_KONG_GATEWAY_HOST -e KONG_PG_USER=INSERT_POSTGRE_KONG_GATEWAY_USERNAME -e KONG_PG_PASSWORD=INSERT_POSTGRE_KONG_GATEWAY_PASSWORD \
